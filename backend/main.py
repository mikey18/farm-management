...
@app.get("/machinery/")
def get_machinery(db: Session = Depends(get_db)):
    return db.query(Machinery).all()

# Livestock Endpoints
@app.post("/livestock/")
def add_livestock(type: str, count: int, birth_date: date, db: Session = Depends(get_db)):
...
# Crop Endpoints
@app.post("/crops/")
def add_crop(name: str, planting_date: date, harvesting_date: date, db: Session = Depends(get_db)):
